  ...
  <Layout>
    ...
    >
          ...
          >
            ...
            <div style="margin-top: 10px">
              ...
              <el-button
                ...
                size="small"
                @click="addProduct"
                v-if="index === purchaseForm.purchaseSheetContent.length - 1"
                >添加</el-button
              >
              <el-button
                type="text"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitForm('purchaseForm')"
          >立即创建</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  mounted() {
    ...
    if (sheet != null) {
      this.purchaseForm = JSON.parse(sheet);
  ...
  methods: {
    ...
    getPurchase() {
      getAllPurchase({}).then((_res) => {
        ...
        this.pendingLevel1List = this.purchaseList.filter(
          (item) => item.state === "待一级审批"
        );
        this.pendingLevel2List = this.purchaseList.filter(
          (item) => item.state === "待二级审批"
        );
        this.successList = this.purchaseList.filter(
          (item) => item.state === "审批完成"
        );
        this.failureList = this.purchaseList.filter(
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          createPurchase(this.purchaseForm)
            .then((_res) => {
              //console.log(_res)
              if (_res.msg === "Success") {
