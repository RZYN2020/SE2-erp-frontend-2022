  ...
  <Layout>
    ...
    <div class="body">
      <el-tabs v-model="activeName" :stretch="true">
        <el-tab-pane label="待一级审批" name="PENDINGw_LEVEL_1">
    ...
    >
      <div style="width: 90%; margin: 0 auto">
        ...
        >
          <el-form-item label="进货单id: " prop="purchaseSheetId">
            <el-select
              v-model="purchaseReturnForm.purchaseSheetId"
              placeholder="请选择关联的进货单id"
              @change="
                selectPurchase(
                  completedPurchase.filter(
                    (item) => item.id === purchaseReturnForm.purchaseSheetId
                  )
                )
              "
            >
              ...
              >
                <el-popover placement="right" width="800" trigger="hover">
                  <el-table
                    :data="completedPurchase[index].purchaseSheetContent"
                  >
                    <el-table-column
                      width="100"
                      property="id"
          ...
          <el-form-item
            label="进货单清单: "
            v-if="
              this.purchaseReturnForm.purchaseReturnsSheetContent.length === 0
            "
          >
            暂无数据!
          </el-form-item>
          <el-form-item label="进货单清单: " v-else>
            <div
              v-for="(
                item, index
              ) in purchaseReturnForm.purchaseReturnsSheetContent"
              :key="index"
            >
              <el-row>
                <el-col>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitForm('purchaseReturnForm')"
          >立即创建</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  async mounted() {
    ...
    if (sheet != null) {
      this.purchaseReturnForm = JSON.parse(sheet);
  ...
  methods: {
    ...
    getPurchaseReturn() {
      getAllPurchaseReturn({}).then((_res) => {
        ...
        this.pendingLevel1List = this.purchaseReturnList.filter(
          (item) => item.state === "待一级审批"
        );
        this.pendingLevel2List = this.purchaseReturnList.filter(
          (item) => item.state === "待二级审批"
        );
        this.successList = this.purchaseReturnList.filter(
          (item) => item.state === "审批完成"
        );
        this.failureList = this.purchaseReturnList.filter(
          (item) => item.state === "审批失败"
        );
      }).catch((err) => {
          this.$message({
            type: "error",
            message: "获取进货退货单列表失败!" + err,
          });
        });;
    },
    handleClose(done) {
    ...
    selectPurchase(content) {
      this.purchaseReturnForm.purchaseReturnsSheetContent =
        content[0].purchaseSheetContent;
    },
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.purchaseReturnForm.purchaseReturnsSheetContent.forEach(
            (item) => {
