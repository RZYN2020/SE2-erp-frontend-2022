  ...
  <Layout>
    ...
    <div>
      ...
      <el-date-picker v-model="date" type="datetimerange" range-separator="至" start-placeholder="开始日期"
        end-placeholder="结束日期" @change="getData()">
      </el-date-picker>
      <div v-if="date !== ''" class="mt15">
        ...
        <div class="mt15">
          ...
          <el-table :data="sheetContent" stripe style="width: 100%" :header-cell-style="{ 'text-align': 'center' }"
            :cell-style="{ 'text-align': 'center' }" class="mt15">
            <el-table-column prop="type" label="类型" width="100"
              :filters="[{ text: '入库', value: 'warehouse_input' }, { text: '出库', value: 'warehouse_output' }]"
              :filter-method="filterTag" filter-placement="bottom-end">
              <template slot-scope="scope">
                <el-tag :type="scope.row.type === 'warehouse_input' ? 'primary' : 'success'" disable-transitions>
                  {{ transform(scope.row.type) }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="sheetId" label="id" width="180">
            </el-table-column>
            <el-table-column prop="productName" label="产品名称" width="180">
            </el-table-column>
            <el-table-column prop="categoryName" label="产品所属分类" width="120">
            </el-table-column>
            <el-table-column prop="quantity" label="数量" width="120">
            </el-table-column>
            <el-table-column prop="unitPrice" label="单价(元)" width="120">
            </el-table-column>
            <el-table-column prop="total_price" label="总价(元)" width="150">
            </el-table-column>
...
import {
  getWarehouseIODetailByTime,
  getWarehouseIPQByTime,
  getWarehouseOPQByTime
